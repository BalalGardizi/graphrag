...
def get_model():
    ...
    model_choice = os.getenv('MODEL_CHOICE', 'qwen3:4b')
    api_key = os.getenv('GOOGLE_API_KEY', 'no-api-key-provided')
    return GoogleModel(model_choice, provider=GoogleProvider(api_key=api_key))

# ========== Create the Graphiti agent ==========
graphiti_agent = Agent(
    get_model(),
    system_prompt="""You are a helpful assistant with access. Show what is the input of your context before answering""",
...
async def main():
    ...
    results = await graphiti_client.search('what is customer001 product')
